...
BookGPT 用来把一份你有合法使用权的中文 `.txt` 文本，转换成可训练数据，并在本地用 `Qwen + Unsloth QLoRA` 做中文写作风格微调。
...
6. 用本地 `Qwen2.5-32B-Instruct-bnb-4bit` 做 LoRA 微调

## 默认假设

- 训练语言：中文
- 默认模型：`unsloth/Qwen2.5-32B-Instruct-bnb-4bit`
- 默认训练长度：`2048`
- 默认训练步数：`320`
- 默认真实 batch size：`32`
